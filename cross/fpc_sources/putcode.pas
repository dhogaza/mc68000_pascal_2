...
function dump_externals;
  ...
  begin {dump_externals}
    ...
    temp := ctr;
    insertobj(temp div $10000); { high order }
    insertobj(temp mod $10000); { low order }
        ...
        with vtptr^ do
          case extvaralloc of
            definealloc:
              ...
              temp := vtptr^.offset;
              insertobj(temp div $10000); { high order offset in psect }
              objtype[objctr] := objoff;
              insertobj(temp mod $10000); { low order offset in psect }
              objtype[objctr] := objoff;
              end;
 ...
 { 68020 scale factor encoding }
  ...
  procedure pic_gen;
    ...
    begin {pic_gen}
      ...
      insertobj($6000 { Emit 3 ESDID's }
                + (ord(mc68020) * $800) {1 word reloc'ed, 2 for 020}
                + (ord(mc68020) * $200 + $200) {2 disp bytes, 4 for 020}
      ...
      if mc68020 then
        ...
        insertobj(( - currentpc + 2) div $10000);
        objtype[objctr] := objoff;
        insertobj(( - currentpc + 4) mod $10000);
        objtype[objctr] := objoff;
      ...
      case m of
        ...
        indexed:
          ...
          if (offset > 127) or (offset < - 128) then
            ...
            if mc68020 then
              ...
              else {generate word offset} insertobj(offset and $FFFF);
              end
            ...
            insertobj(indxr * 10000B + ord(indxlong) * 4000B +
                      (scale_factor[scale] * 1000B) + (offset and $FF));
        ...
        commonlong:
          ...
          if commonlong_reloc < 0 then
            ...
            insertobj(temp div $10000);
            objtype[objctr] := objoff;
            insertobj(temp mod $10000);
            objtype[objctr] := objoff;
            end;
...
{ note: although this operand will be 4 bytes long when eventually loaded,
  ...
  and the line will be flagged "EXTERNAL".
            ...
            if procmap[offset].addr = undefinedaddr then
              ...
              insertobj(temp div $10000); {high order}
              objtype[objctr] := objoff;
              insertobj(temp mod $10000); { fixup adds proctable addr }
              objtype[objctr] := objoff;
              allocfixup;
              fixups[objctr - 1] := fixuptail;
              with fixuptail^ do
              ...
              objtype[objctr] := objforw;
              relocn[objctr] := true;
              insertobj((procmap[offset].addr + offset1) div $10000);
              { ^^^ high order }
              objtype[objctr] := objoff;
              currentpc := currentpc - 2; {this stuff's longer than code}
              insertobj((procmap[offset].addr + offset1) mod $10000);
              objtype[objctr] := objoff;
              end
            else
        ...
        pic_usercall:
          ...
          if proctable[offset].externallinkage and
            ...
            insertobj(( - (offset1 - 2) - currentpc) div $10000);
            objtype[objctr] := objoff;
            insertobj(( - (offset1 - 2) - currentpc + 2) mod $10000);
            objtype[objctr] := objoff;
            end
        ...
        pic_branch:
          ...
          insertobj((labelpc - (offset1 - 2) - currentpc) div $10000);
          insertobj((labelpc - (offset1 - 2) - currentpc + 2) mod $10000);
        ...
        pic_pcrelative:
          ...
          mode := 74B; {immediate}
          op := op + mode;
          insertobj((offset - (offset1 - 2) - currentpc) div $10000);
          insertobj((offset - (offset1 - 2) - currentpc + 2) mod $10000);
          end;
        end; { case m }
  end; {setmodeonly}

  ...
  begin
    if n^.kind = labelnode then
      ...
      if n^.labelcost = long then { 68020 and pic only }
        begin
        op := $FF + op;
        insertobj((labelpc - currentpc) div $10000);
        insertobj((labelpc - currentpc + 2) mod $10000);
  ...
  begin
    if n^.kind = labelnode then
      ...
      if n^.labelcost = long then { 32 bit branch }
        ...
        insertobj(distancetemp div $10000);
        insertobj(distancetemp mod $10000);
