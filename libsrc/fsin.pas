{ NOTICE OF COPYRIGHT AND OWNERSHIP OF SOFTWARE:
  ...
  Single Precision Real Sine and Cosine Functions

 Last modified by KRIS on 26-Nov-1990 13:56:47
 Purpose:
...
{$ident='Single Precision Real Sine and Cosine Functions'}
...
function p_fsin {( realval: real) : real} ;
  ...
  begin
    ...
    realval := abs(realval);
    if realval > xMax then p_fsin := 0.0
    else
      begin
      ...
      if negative then p_fsin := - r
      else p_fsin := r;
      end;
  end;


function p_fcos {( realval: real) : real} ;
