{ NOTICE OF COPYRIGHT AND OWNERSHIP OF SOFTWARE:
  ...
  Single precision real to char conversion routines

 Last modified by KRIS on 26-Nov-1990 13:55:37
 Purpose:
...
{$ident='Single precision real to char conversion routines'}
...
const
  ...
  maxexpon = + 39;
  minexpon = - 39;
  minexpm1 = - 40; {required for certain data declarations}
  ExpDigits = 2; {implementation-defined number of decimal digits in exponent}
  maxbitlength = 149; { fraction with 125 leading 0's + 24 significant bits }
  maxbinfrac = 19; {maximum number of "hosthalfword" required to contain the
                    longest real fraction. (Maxbitlength + 7) div 8. }
  bitsperbyte = 8;
  maxrealwords = 2; {number of host unsigned integers per target real}
  totalfracbits = 24; {number of bits in significand including implied bit}
  totalfracbytes = 3; {(totalfracbits + bitsperbyte - 1) div bitsperbyte}
  impliedbit = 128; {added into high order byte if non zero exponent}
  exponbias = 127; {target representation exponent bias}
...
type
  ...
  realblock = { a structure containing a decimal integer stack and a variable
    ...
    record
      digitctr: minexpm1..maxexpon; {counts integer digits if greater than
                                     zero, and leading zeroes if negative}
      decstring: packed array [1..maxexpon] of char; {integer digit stack}
...
{ this procedure accepts a real number, unpacks it into a multiprecision
  ...
  begin {scale}
    ...
    with rb do
      ...
      if (x.e = 0) and (x.f1 = 0) and (x.f[2] = 0) then leadingzeroes := 0
      else leadingzeroes := x.e - (exponbias - pointoffset - ord(x.e = 0));

      shift := leadingzeroes mod bitsperbyte;
      ...
      if leadingzeroes < 0 then
        begin {there is no integer part}
        leadingzeroes := ( - leadingzeroes) div bitsperbyte;

        for i := 1 to leadingzeroes do
          begin
          tail := tail + 1;
          binfrac[tail] := 0;
          end;
      ...
      else point := (leadingzeroes + bitsperbyte - 1) div bitsperbyte;
      ...
      if x.e <> 0 then {set implied bit if non-zero exponent}
        binfrac[tail] := binfrac[tail] + impliedbit;
...
{- - -  shift the fraction left to align binary point on a byte boundary - - -}
      ...
      if point = 0 then
        ...
        if (x.e <> 0) or (x.f1 <> 0) or (x.f[2] <> 0) then
...
procedure p_ftoc1 { ( { procedure putf(ch: char); {receives chars of converted
                   value}
 ...
 {                  ) } ;
  ...
  begin {p_ftoc1}
    p_entlib;
    x := loophole(reel, value); { to access exponent and sign bit}
    if x.s = 1 then putf('-')
    else putf(' ');
    ...
    if x.e = 255 then
      ...
      putf(chr(abs(ExpValue) div 10 + ord('0')));
...
procedure p_ftoc2 { ( { procedure putf(ch: char); {receives chars of converted
                   value}
 {    value     : real;         {the real value to convert}
 {    total_width: integer;      {total field width for conversion}
