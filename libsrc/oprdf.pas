{ NOTICE OF COPYRIGHT AND OWNERSHIP OF SOFTWARE:
  ...
  Single precision real input routine

 Last modified by KRIS on 26-Nov-1990 13:55:04
 Purpose:
...
{$ident='Single precision real input routine'}
{$nomain,nopointercheck,norangecheck,noindexcheck,nowalkback}

{
  p_rdf -- write(file, real, width, fracs)
  p_rdf_i -- write(real, width, fracs)
}
%include opcom;
%include pasers;
...
{  This is the procedure from which p_ctof will take a character
  ...
  begin {p_rdf}
    p_entlib; {no walkback for this module}
    first_flag := true;
    p_ctof(nextch, real_value, errcode); {let format the real}
    case errcode of
      noerror: p_rdf := real_value;
      syntaxerror: p_seterr(f, ord(fisyntax), p_stxfer);
      underflowerror: p_rdf := 0.0;
