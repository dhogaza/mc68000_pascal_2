{ NOTICE OF COPYRIGHT AND OWNERSHIP OF SOFTWARE:
  ...
  Single Precision Output Interface (one format parameter)

 Last modified by KRIS on 26-Nov-1990 13:55:11
 Purpose:
...
{$ident='Single Precision Output Interface (one format parameter)'}
{$nomain,nopointercheck,norangecheck,noindexcheck,nowalkback}

%include opcom;

{
  p_wtf1 -- write(file, real, width)
  p_wtf1_o -- write(real, width)
}
...
procedure p_wtf1(var f: text;
                 val: real;
                 width: integer);
  external;


procedure p_wtf1_o(val: real;
                   width: integer);
  external;
...
{  This is the procedure to which p_ftoc1 will pass a character
   for output.  Characters are buffered up and the string output
   routine is used instead of single character i/o. The buffer
   is defined as a local variable on the p_wtf1's stack space.}
  ...
  begin {p_wtf1}
    p_entlib; {no walkback for this module}
    position := 0;
    p_ftoc1(putch, val, width); {let format the real}
